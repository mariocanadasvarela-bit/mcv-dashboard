// app.js - MCV Seguros Dashboard - Versión final con tarjetas de ramos + pólizas
let polizasData = [];
let itemsData = [];
let charts = {};
...
// ======================== DATOS EMBEBIDOS ========================
const datosPolizas = [
    // RECURRENTES (estructura operativa)
    ...
    // EVENTOS (Expo San Juan Minero 2026)
    { id_poliza: 'EV-001', nro_poliza: '12-10363561', tomador: 'GHM Satelital SRL', empresa: 'GHM Satelital SRL', ramo: 'Accidentes Personales', aseguradora: 'Federación Patronal', costo_mensual_ars: 16522.72, costo_mensual_usd: 0, cobertura_corta: 'Accidentes Personales', suma_asegurada_total: 40000000, cantidad_items: 0, vigencia_desde: '04/05/2026', vigencia_hasta: '10/05/2026', proyecto_norm: 'expo san juan minero 2026', esCaucion: false, costo_total_vigencia: null },
...
function getPolizasFiltradas() {
    let base = currentTab === 'recurrente' ? polizasData.filter(p => p.proyecto_norm === 'estructura operativa') : polizasData.filter(p => p.proyecto_norm === 'expo san juan minero 2026');
...
function renderRamosCards(polizas) {
    ...
    for (const [ramo, pols] of ramosMap.entries()) {
        ...
        card.addEventListener('click', () => {
            selectedRamo = ramo;
            const polsRamo = polizas.filter(p => p.ramo === selectedRamo);
            renderPolizasDeRamo(polsRamo);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('exportItemsBtn')?.addEventListener('click', () => alert('Exportar bienes del modal (funcionalidad por desarrollar)'));
    ...
    // Asegurar que "Ambas" esté activo al inicio
