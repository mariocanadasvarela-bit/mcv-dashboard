// app.js - MCV Seguros Dashboard - Con carga de CSVs + correcciones embebidas
...
// ======================== CORRECCIONES (sobrescriben datos de los CSVs) ========================
const correcciones = new Map();

// Formato de corrección:
// clave = número real de póliza (string)
// valor = objeto con campos a corregir: premio_ars, premio_usd, suma_asegurada_total, cobertura_corta, vigencia_desde, vigencia_hasta, items (array)
...
function renderChart(canvasId, chartKey, dataObj, label) {
    ...
    if (chartTypes[chartKey] === 'pie') {
        ...
        charts[chartKey] = new Chart(ctx, { type: 'bar', data: { labels, datasets: [{ label, data: values, backgroundColor: '#a5f3fc', borderRadius: 8 }] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: true, onClick: (e, activeEls) => { if (activeEls.length) { const ramo = labels[activeEls[0].index]; currentRamoFiltro = ramo; renderCurrentTab(); document.getElementById('polizasDetailContainer').scrollIntoView({ behavior: 'smooth' }); } } } });
    }
}
...
function renderRamoCards(polizas) {
    ...
    for (const [ramo, pols] of ramosMap.entries()) {
        let gastoARS = 0, gastoUSD = 0, sumaAsegurada = 0, allItems = [];
        for (const p of pols) {
    ...
    for (const rd of ramosArray) {
        ...
        card.addEventListener('click', () => { currentRamoFiltro = rd.ramo; currentPolizaSearch = ''; document.getElementById('searchPoliza').value = ''; renderCurrentTab(); document.getElementById('polizasDetailContainer').scrollIntoView({ behavior: 'smooth' }); });
        const icon = getIconForRamo(rd.ramo);
        card.innerHTML = `<div class="flex items-center justify-between mb-1"><div class="flex items-center gap-1"><i class="${icon} text-sm text-[#38bdf8]"></i><span class="font-semibold text-sm">${rd.ramo}</span></div><span class="text-xs bg-gray-100 px-1.5 py-0.5 rounded-full">${rd.cantidadPolizas} pólizas</span></div><div class="text-xs text-gray-600 mb-1">Gasto: ${rd.gastoARS>0 ? 'ARS '+rd.gastoARS.toLocaleString() : ''}${rd.gastoARS>0 && rd.gastoUSD>0 ? ' + ' : ''}${rd.gastoUSD>0 ? 'USD '+(rd.gastoUSD/tipoCambioUSD).toLocaleString() : ''}</div><div class="text-xs text-gray-500 truncate" title="${rd.resumenBienes}">${rd.resumenBienes}</div>`;
...
function renderPolizasDetail(polizasFiltradas) {
    ...
    for (const poliza of polizasFiltradas) {
        ...
        header.innerHTML = `<div class="flex items-center gap-2"><i class="fas fa-chevron-right text-gray-400 expand-icon-politica"></i><div><div class="font-mono text-sm font-bold text-[#1e293b]">${displayNumber}</div><div class="text-xs text-gray-500">${poliza.aseguradora}</div></div></div><div class="flex flex-wrap gap-3 text-xs"><div><span class="text-gray-500">Cobertura:</span> <span class="font-medium">${poliza.cobertura_corta || 'No especificada'}</span></div><div><span class="text-gray-500"><i class="fas fa-dollar-sign text-xs"></i> ARS:</span> ${poliza.costo_mensual_ars ? poliza.costo_mensual_ars.toLocaleString() : '0'}</div>${poliza.costo_mensual_usd ? `<div><span class="text-gray-500"><i class="fas fa-dollar-sign text-xs"></i> USD:</span> ${poliza.costo_mensual_usd.toLocaleString()}</div>` : ''}<div><span class="text-gray-500"># Bienes:</span> ${poliza.cantidad_items}</div></div>`;
        const body = document.createElement('div');
        body.className = 'hidden border-t border-[#e2e8f0] p-3 bg-gray-50';
        if (items.length === 0) body.innerHTML = '<p class="text-gray-500 text-sm">No hay bienes registrados.</p>';
        else {
            ...
            for (const item of items) tableHtml += `<tr class="border-b"><td class="py-1">${item.item||'-'}</td><td class="py-1">${item.identificacion||'-'}</td><td class="py-1 text-right">${item.suma_asegurada_ars ? '$'+item.suma_asegurada_ars.toLocaleString() : '$0'}</td><td class="py-1 text-right">${item.suma_asegurada_usd ? '$'+item.suma_asegurada_usd.toLocaleString() : '-'}</td></tr>`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.empresa-btn').forEach(btn => btn.addEventListener('click', () => {
        ...
        else { btn.classList.add('bg-[#fef9c3]'); currentEmpresa = 'GHM Satelital SRL'; }
