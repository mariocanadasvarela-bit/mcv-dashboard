// app.js - MCV Seguros Dashboard - Versión final con estilo premium, tabla mejorada, cauciones como KPI aparte
let polizasData = [];
let itemsData = [];
let charts = {};
...
// Datos embebidos (los mismos que ya funcionaban, pero añadimos campo "esCaucion")
const datosPolizas = [
    ...
    // Cauciones: esCaucion = true, costo_total_vigencia = suma pagada por toda la vigencia
...
function sortPolizas(polizas) {
    return polizas.sort((a, b) => {
        ...
        switch (sortColumn) {
            ...
            case 'costo': valA = esCaucion(a) ? a.costo_total_vigencia : a.costo_mensual_ars; valB = esCaucion(b) ? b.costo_total_vigencia : b.costo_mensual_ars; break;
            case 'costo_usd': valA = esCaucion(a) ? (a.moneda_total === 'USD' ? a.costo_total_vigencia : 0) : a.costo_mensual_usd; valB = esCaucion(b) ? (b.moneda_total === 'USD' ? b.costo_total_vigencia : 0) : b.costo_mensual_usd; break;
...
function updateKPIs(polizas) {
    // Para gasto mensual: solo pólizas que NO son cauciones y tienen costo_mensual_ars o usd
    ...
    // Suma asegurada total (todas las pólizas)
    let sumaAsegurada = 0;
    ...
    // Cauciones: suma de costos totales de vigencia (solo cauciones)
...
function updateCharts(polizas) {
    // Para gráficos, solo pólizas que no son cauciones
    const polizasMensuales = polizas.filter(p => !esCaucion(p));
...
function renderTablaPolizas(polizas) {
    ...
    table.innerHTML = `
        <thead class="bg-slate-50">
            <tr class="border-b border-slate-200">
                <th class="px-5 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100" data-col="nro_poliza">Nº Póliza ${sortColumn === 'nro_poliza' ? (sortDirection === 'asc' ? '↑' : '↓') : ''}</th>
                <th class="px-5 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100" data-col="aseguradora">Aseguradora</th>
                <th class="px-5 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100" data-col="ramo">Ramo</th>
                <th class="px-5 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100" data-col="cobertura">Cobertura</th>
                <th class="px-5 py-4 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100" data-col="costo">Costo</th>
                <th class="px-5 py-4 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100" data-col="suma_asegurada">Suma asegurada</th>
                <th class="px-5 py-4 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100" data-col="cantidad_items"># Bienes</th>
                <th class="px-5 py-4 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">Acción</th>
            </tr>
        </thead>
    ...
    for (const poliza of polizas) {
        ...
        // Determinar texto de costo
        let costoTxt = '';
        if (esCaucion(poliza)) {
            if (poliza.moneda_total === 'ARS') costoTxt = `${abreviaturaNumero(poliza.costo_total_vigencia)} ARS (total vigencia)`;
            else costoTxt = `${abreviaturaNumero(poliza.costo_total_vigencia)} USD (total vigencia)`;
        } else {
            let partes = [];
            ...
            costoTxt = partes.join(' + ') || '$0';
        }
        row.innerHTML = `
            <td class="px-5 py-3 text-sm font-mono font-medium text-slate-700">${poliza.nro_poliza}</td>
            <td class="px-5 py-3 text-sm text-slate-600">${poliza.aseguradora}</td>
            <td class="px-5 py-3 text-sm text-slate-600">${poliza.ramo}</td>
            <td class="px-5 py-3 text-sm text-slate-600 max-w-xs truncate" title="${poliza.cobertura_corta}">${poliza.cobertura_corta}</td>
            <td class="px-5 py-3 text-sm text-right font-medium text-slate-700">${costoTxt}</td>
            <td class="px-5 py-3 text-sm text-right font-mono text-slate-600">${abreviaturaNumero(poliza.suma_asegurada_total)}</td>
            <td class="px-5 py-3 text-sm text-center text-slate-600">${poliza.cantidad_items}</td>
            <td class="px-5 py-3 text-sm text-center"><button class="ver-bienes-btn bg-indigo-50 hover:bg-indigo-100 text-indigo-700 px-3 py-1.5 rounded-lg text-xs font-medium transition" data-id="${poliza.id_poliza}">Ver bienes</button></td>
        `;
    }
    ...
    // Eventos para botones "Ver bienes" -> abrir modal
