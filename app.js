...
function normalizarTexto(str) {
    if (!str) return '';
    return str.toString().trim().replace(/\s+/g, ' ').toLowerCase().replace(/\b\w/g, l => l.toUpperCase());
}
...
async function loadAllData() {
    ...
    if (polCSV && polCSV.length) {
        polizasData = polCSV.map(p => {
            let proyecto = normalizarTexto(p['Proyecto / Centro de Costo']);
            let ramo = normalizarTexto(p.Ramo);
            let aseguradora = normalizarTexto(p.Aseguradora);
            return {
                id_poliza: p.ID_Poliza?.toString().trim() || '',
                tomador: normalizarTexto(p.Tomador),
                proyecto: proyecto,
                ramo: ramo,
                aseguradora: aseguradora,
                vigencia_hasta: p['Vigencia Hasta (fecha)']?.trim() || '',
    ...
    if (itemsCSV && itemsCSV.length) {
        itemsData = itemsCSV.map(i => ({
            ...
            valor_asegurado: parseFloat(i.Valor_Asegurado || 0)
        }));
    } else {
    ...
    polizasData = polizasData.map(p => {
        ...
        let exposicion = itemsPol.reduce((sum, i) => sum + i.valor_asegurado, 0);
        if (exposicion === 0) {
    ...
    polizasData = polizasData.map(p => {
        ...
        if (empresa === 'Ghm Contenidos') empresa = 'GHM SRL';
        if (empresa !== 'GHM SRL' && empresa !== 'GHM Satelital SRL' && empresa.includes('Ghm')) empresa = 'GHM SRL';
        return { ...p, empresa };
    });
...
const esRecurrente = (p) => p.proyecto === 'Estructura Operativa';
const esEvento = (p) => p.proyecto === 'Expo San Juan Minero 2026';
...
function updateResumenEmpresas() {
    ...
    data.forEach(d => {
        ...
        tbody.innerHTML += `<tr><td class="py-2">${d.empresa}</td><td>$${d.gasto.toLocaleString()}</td><td>$${d.patrimonio.toLocaleString()}</td><td>${d.polizas}</td><td>${porcentaje}%</td></tr>`;
    });
    ...
    charts.gastoEmpresa = new Chart(ctx, {
        type: 'doughnut',
        data: { labels: data.map(d => d.empresa), datasets: [{ data: data.map(d => d.gasto), backgroundColor: ['#2c5f8a', '#4a8cbd'] }] },
        options: { plugins: { legend: { position: 'bottom', labels: { color: '#e2e8f0' } } }, responsive: true, maintainAspectRatio: true }
    });
}
...
function updateTablaPolizas() {
    ...
    filteredPolizas.forEach(p => {
        ...
        const venc = p.vigencia_hasta ? dayjs(p.vigencia_hasta, 'DD/MM/YYYY').format('DD/MM/YYYY') : '-';
        tbody.innerHTML += `<tr>
            ...
            <td class="px-4"><span class="text-xs px-2 py-0.5 rounded-full bg-[#2a3a4f] print:bg-gray-200">${tipo}</span></td>
        </tr>`;
    });
}
...
function updateChartsRecurrentes() {
    ...
    charts.gastoAseguradora = new Chart(document.getElementById('chartGastoAseguradora'), {
        type: 'bar', data: { labels: Object.keys(gastoAseguradora), datasets: [{ label: 'Gasto mensual ARS', data: Object.values(gastoAseguradora), backgroundColor: '#2c5f8a', borderRadius: 8 }] },
        options: { indexAxis: 'y', responsive: true, maintainAspectRatio: true, plugins: { legend: { labels: { color: '#e2e8f0' } } }, scales: { x: { ticks: { color: '#94a3b8' }, grid: { display: false } }, y: { ticks: { color: '#94a3b8' }, grid: { display: false } } } }
    });
    const gastoRamo = {};
    ...
    charts.gastoRamo = new Chart(document.getElementById('chartGastoRamo'), {
        type: 'doughnut', data: { labels: Object.keys(gastoRamo), datasets: [{ data: Object.values(gastoRamo), backgroundColor: ['#2c5f8a','#4a8cbd','#6ba3d6','#1e3a5f','#3b7a9e'] }] },
        options: { plugins: { legend: { position: 'bottom', labels: { color: '#e2e8f0' } } }, responsive: true, maintainAspectRatio: true }
    });
    ...
    charts.patrimonioAseguradora = new Chart(document.getElementById('chartPatrimonioAseguradora'), {
        type: 'bar', data: { labels: Object.keys(patrimonioAseguradora), datasets: [{ label: 'Patrimonio ARS', data: Object.values(patrimonioAseguradora), backgroundColor: '#2c5f8a', borderRadius: 8 }] },
        options: { indexAxis: 'y', responsive: true, maintainAspectRatio: true, plugins: { legend: { labels: { color: '#e2e8f0' } } }, scales: { x: { ticks: { color: '#94a3b8' }, grid: { display: false } }, y: { ticks: { color: '#94a3b8' }, grid: { display: false } } } }
    });
    ...
    charts.patrimonioRamo = new Chart(document.getElementById('chartPatrimonioRamo'), {
        type: 'doughnut', data: { labels: Object.keys(patrimonioRamo), datasets: [{ data: Object.values(patrimonioRamo), backgroundColor: ['#2c5f8a','#4a8cbd','#6ba3d6','#1e3a5f','#3b7a9e'] }] },
        options: { plugins: { legend: { position: 'bottom', labels: { color: '#e2e8f0' } } }, responsive: true, maintainAspectRatio: true }
    });
}
...
function getSamplePolizas() {
    return [
        ...
        { id_poliza: "POL-022", tomador: "GHM SRL", proyecto: "Estructura Operativa", ramo: "Caución", aseguradora: "Aseguradores de Cauciones", vigencia_hasta: "11/08/2026", costo_mensual_ars: 165600, costo_mensual_usd: 138 }
    ];
}
function getSampleItems() {
    return [
        { id_poliza: "POL-001", valor_asegurado: 45000000 },
        { id_poliza: "POL-022", valor_asegurado: 80000000 }
    ];
}
